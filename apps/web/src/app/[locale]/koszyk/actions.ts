...
export async function placeOrder(formData: FormData, cartItemsJson: string, total: number) {
    ...
    // Fetch Base Custom Product if needed
    let customBaseProduct: { id: string, price: number, name: string } | null = null;
    if (hasCustomItems) {
        customBaseProduct = await prisma.product.findUnique({
        ...
        if (!customBaseProduct) {
            customBaseProduct = await prisma.product.create({
                data: {
    ...
    for (const item of items) {
        ...
        if (!dbProduct && (item.configuration || item.id.startsWith('config-'))) {
            dbProduct = customBaseProduct!;
            isCustom = true;
        }
        ...
        // For custom items, we trust the client price for now (or could re-calculate from config)
        // Ideally we should re-fetch settings and calculate. 
        // For MVP, we use the passed price if it is reasonable or fallback to base.
        // But `item.price` comes from client.
        const effectivePrice = isCustom ? item.price : dbProduct.price;

        const itemTotal = effectivePrice * item.quantity;
        calculatedItemsTotal += itemTotal;

        trustedItems.push({
            productId: dbProduct.id, // Maps custom items to Base Product ID
            name: isCustom ? item.name : dbProduct.name, // Keep custom name (e.g. "Bombka Red")
